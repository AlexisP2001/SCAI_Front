<template>
    <v-app>
        ...
        <template>
            ...
            <!-- boton busqueda -->
            <v-btn style="margin-right:10px;  margin-top:20px"   icon color="#72128E"  @click="obtenerCreditos()"><v-icon size="40">mdi-card-search-outline</v-icon> </v-btn>
              <!--boton de excel-->
            ...
            <v-btn style="margin-right:10px; margin-left:20px;  margin-top:20px"  class="mb-2 purple darken-3 white--text"   @click="obtenerCreditos()">Todos</v-btn>
        </v-row>  
        </v-row>
        </v-card> 
      </template>
    <template>
        <!--tabla de categorias-->
        <v-data-table style="margin-top:50px"  class="elevation-15 "  :headers="columnas" :items="creditos"  :search="search">
          <template v-slot:top>
            <v-toolbar  flat>
  ...
  export default {
    data: () => ({    
      ...
      columnas: [
        ...
        { text: 'Factura', value: 'numFacturaVenta'  ,class:'purple darken-3 white--text'},
        { text: 'Tipo', value: 'subTipoFactura'  ,class:'purple darken-3 white--text'},
        ...
        { text: 'Pago Credito', value: 'abonoCredito'  ,class:'purple darken-3 white--text'},
        { text: 'Pago Efectivo', value: 'abonoEfectivo'  ,class:'purple darken-3 white--text'},
    ...
    methods: {
      ...
      obtenerCreditos(){
        ...
        axios.get(`movimiento/credito?subTipoFactura=${this.subTipoFactura}&fechaInicial=${this.fechaInicio}&fechaFinal=${this.fechaFinal}`,header)
          .then(response =>{
      ...
      limpiarFechas(creditos){
        let arrayTemporal = [];
        creditos.map(function(x){
