<template>
  ...
  <!--boton de excel-->
    ...
    <template>
        <!--tabla de categorias-->
        <v-data-table style="margin-top:50px"  class="elevation-15 "  :headers="columnas" :items="abonos"  :search="search">
          <template v-slot:top>
            <v-toolbar  flat>
  ...
  export default {
    ...
    methods: {
      ...
      obtenerAbonos(){
        ...
        axios.get(`movimiento/abonoAbono?fechaInicial=${this.fechaInicio}&fechaFinal=${this.fechaFinal}`,header)
          .then(response =>{
